...
import { useChatRoom, getAuthToken } from '@/src/components/useChatRoom';
...
export default function CustomerChatPage() {
  ...
  const [riderMeta, setRiderMeta] = useState<{ otherUserId: string; otherUserName: string }>({
    otherUserId: '',
    otherUserName: 'Assigned Rider',
  });

  useEffect(() => {
    async function resolveTrackingCode() {
      if (!trackingCode) {
        setResolutionError('Missing tracking identification reference.');
      ...
      try {
        ...
        const shipmentData: any = await ShipmentService.getShipment(trackingCode, token);
        const databaseId = shipmentData?._id || shipmentData?.id || trackingCode;

        setResolvedJobId(databaseId);
        setRiderMeta({
          otherUserId: shipmentData?.rider?.id || shipmentData?.riderId || '',
          otherUserName: shipmentData?.rider?.name || shipmentData?.riderName || 'Assigned Rider',
        });
      } catch (err: any) {
        console.error('Failed to resolve tracking code:', err);
        setResolutionError(err?.message || 'Unable to retrieve shipment details.');
      } finally {
        setIsResolving(false);
      }
    }
  ...
  const displayedError = resolutionError || chatError;
  if (displayedError) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[60vh] bg-slate-50 p-6 text-center">
        ...
        <p className="text-xs text-slate-500 mt-1 max-w-xs">{displayedError}</p>
        <Link href={`/orders/${trackingCode}`} className="mt-4 px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-semibold">
          Back to Order
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col h-full min-h-0 bg-slate-50 text-slate-900 font-sans relative overflow-hidden">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-3.5 flex items-center justify-between shrink-0 shadow-xs z-10">
        <div className="flex items-center gap-3">
          <Link href={`/orders/${trackingCode}`} className="p-2 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200 transition">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 flex items-center gap-1">
                <span>Order #{trackingCode.slice(0, 8)}</span>
                <span>•</span>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id || `${msg.timestamp}-${Math.random()}`} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
      ...
      {/* Sticky Bottom Section (Presets + Input Container) properly aligned inside the dashboard viewport */}
