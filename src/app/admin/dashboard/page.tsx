...
export default function AdminControlTower() {
  ...
  // ============================================================================
  // EFFECT 1: REAL-TIME OPERATIONS GATEWAY (SOCKET.IO TO PORT 5000)
  // ============================================================================
  useEffect(() => {
    ...
    // WebSocket link-layer hook for live metrics adjustments
  ...
  // ============================================================================
  // EFFECT 2: REST CORE RUNTIME DATA HYDRATION ROUTER
  // ============================================================================
  useEffect(() => {
    async function fetchTabMetricsData() {
      try {
        setIsLoading(true);
        setError(null);

        if (currentTab === 'OVERVIEW') {
          const res = await fetch(`${BACKEND_URL}/admin/dashboard/overview`);
          if (!res.ok) throw new Error('Could not synchronize system control metrics.');
          ...
          if (!res.ok) throw new Error('Global Manifest ingestion stream failed.');
          const data = await res.json();
          ...
          const res = await fetch(`${BACKEND_URL}/admin/riders/pending-kyc?page=${page}&limit=20`);
          if (!res.ok) throw new Error('Failed to retrieve processing node vector records.');
          const data = await res.json();
          setPendingKYC(data || []);
        }
  ...
  const handleEvaluateKYC = async (applicationId: string, approve: boolean) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorDetails.message || 'KYC application determination modification rejected.');
      }
      ...
      alert(`Critical KYC Override Error: ${err.message}`);
    } finally {
  ...
  const handleSavePricingConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorDetails = await response.json();
        throw new Error(errorDetails.message || 'Configuration matrix deployment failed.');
      }

      const resData = await response.json();
      alert(resData.message || 'Pricing configurations updated successfully.');
      
      // Pull down updated workspace telemetry immediately following mutations
      const resMetrics = await fetch(`${BACKEND_URL}/admin/dashboard/overview`);
      if (resMetrics.ok) setMetrics(await resMetrics.json());
    } catch (err: any) {
      alert(`Pricing Engine Mutation Failure: ${err.message}`);
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-neutral-50 text-neutral-900 font-mono text-xs">
        <div className="animate-pulse tracking-widest text-center">
          ...
          <p className="text-[10px] text-neutral-400 mt-1">Hydrating operational metrics from port 5000 parameters</p>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 text-neutral-900 antialiased font-mono text-xs">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between border-b border-neutral-200 pb-5 gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-neutral-400 font-sans mt-1">Live administrative terminal synchronized with port 5000 cache architectures.</p>
        </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-150">
          ...
          <div className="bg-neutral-50 rounded-2xl p-6 border text-center space-y-2 max-w-2xl mx-auto">
            ...
            <p className="text-neutral-500 text-[11px] font-sans">
              Operational vectors auto-hydrate from port 5000 core processes every 60 seconds or recalculate instantly when dynamic actions change database configuration models.
            </p>
          </div>
        </div>
      )}
      ...
      {currentTab === 'PRICING_ENGINE' && (
        <div className="max-w-2xl mx-auto border border-neutral-200 rounded-xl bg-white shadow-sm overflow-hidden animate-in fade-in duration-150">
          <div className="p-4 border-b border-neutral-100 bg-neutral-50">
            ...
            <p className="text-[11px] text-neutral-400 font-sans">Updates here trigger transaction mutations inside database rows and recalculate `forceHydrate()` targets.</p>
          </div>
