...
export default function LiveMap({ shipment }: { shipment: ShipmentDetails }) {
  ...
  // Use the TRUE hardware position if available, only fallback to default variables if searching
  const activeLat = myLocation?.lat ?? pickupLat;
  const activeLng = myLocation?.lng ?? pickupLng;

  // 1. HARDWARE TELEMETRY ENGINE: Force real high-accuracy phone coordinates updates
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      ...
      {
        enableHighAccuracy: true, // Forces phone to boot up hardware GPS satellites instead of cell towers
        timeout: 10000,
        maximumAge: 0,
      }
    );
  ...
  // 2. HARDWARE COMPASS ENGINE: Listen to real physical device gyroscopes turns
  useEffect(() => {
  ...
  // Initialize Dynamic Socket connections
  useEffect(() => {
  ...
  // Stream true coordinates straight to your Port 5000 backend server instance
  useEffect(() => {
  ...
  // Calculate high-fidelity road lines path configurations
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-full min-h-[420px]">
      {/* Visual Diagnostic Banner for Testing Verification */}
      <div className="absolute top-4 left-4 z-[1000] bg-black/85 backdrop-blur-md px-3 py-2 rounded-lg text-[11px] font-mono border border-neutral-800 text-white flex flex-col gap-1 shadow-xl">
        ...
        {gpsError && <div className="text-rose-400 font-bold max-w-[200px]">{gpsError}</div>}
      ...
      {/* Increased Zoom level to 17 mirroring close proximity mobile configurations */}
      <MapContainer center={[activeLat, activeLng]} zoom={17} zoomControl={false} className="w-full h-full min-h-[420px]">
        
        {/* UPDATED: Crisp Light Map styling showing ALL street labels, grids, and boundaries perfectly */}
        <TileLayer
